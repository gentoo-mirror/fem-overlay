...
src_install() {
	...
	for PROFILE in all default gentoo ; do
		rm -rf "server/${PROFILE}/deploy/jboss-web.deployer/ROOT.war" || die "rm failed"
		cp -rf gentoo.war "server/${PROFILE}/deploy/" || die "cp failed"
		cp -rf ROOT.war    "server/${PROFILE}/deploy/jboss-web.deployer/" || die "cp failed"
		# our tomcat configuration to point to our helper
		cp -rf "${FILESDIR}/${PV}/tomcat/server.xml" \
			"server/${PROFILE}/deploy/jboss-web.deployer/server.xml"\
			|| die "cp failed"
	done
	...
	for PROFILE in all default gentoo minimal; do
		...
		dosym "${SERVICES_DIR}/${PROFILE}/deploy/jboss-web.deployer/server.xml" \
				"${CONF_INSTALL_DIR}/${DEFAULT_VHOST}/${PROFILE}"
	done
