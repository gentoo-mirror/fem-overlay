...
SRC_URI="http://nginx.org/download/${P}.tar.gz
	...
	nginx_modules_rtmp? ( ${RTMP_MODULE_URI} -> ${RTMP_MODULE_P}.tar.gz )
	nginx_modules_rtmp_hls? ( ${RTMP_MODULE_URI} -> ${RTMP_MODULE_P}.tar.gz )"
...
src_configure() {
	...
	if use nginx_modules_rtmp_hls; then
		http_enabled=1
		myconf+=" --add-module=${RTMP_MODULE_WD}/hls"
	fi
...
src_install() {
	...
	cp "${FILESDIR}"/stat.xsl "${ED}"/etc/nginx/stat.xsl || die
