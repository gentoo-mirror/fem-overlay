...
src_prepare() {
	...
	sed -i \
		...
		-e '/DD/s/1\\.0/ 1\\.0/' \
		-e '/^OLD_LANG/s/OLD_LANG="\$LANG"/OLD_LANG=/' \
		configure
}
...
src_configure() {
	...
	# don't overide settings from qmail-scanner.conf
...
src_install () {
	...
	dohtml README.html FAQ.php TODO.php configure-options.txt manual-install.php perlscanner.php
