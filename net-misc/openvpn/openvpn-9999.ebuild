...
src_install() {
	...
	keepdir /etc/openvpn-dev
	exeinto /etc/openvpn-dev
	...
	doenvd "${FILESDIR}/65openvpn-dev" # config-protect easy-rsa
	if ! use minimal ; then
		cd easy-rsa/2.0
		make install "DESTDIR=${D}" "PREFIX=${EPREFIX}/usr/share/${PN}-${SLOT}/easy-rsa"
		cd ../..
