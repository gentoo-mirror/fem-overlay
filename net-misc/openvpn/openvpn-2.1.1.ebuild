...
SRC_URI="http://openvpn.net/release/${P}.tar.gz
		ipv6? (
			http://cloud.github.com/downloads/jjo/openvpn-ipv6/openvpn-2.1_rc22-ipv6-${IPV6_VERSION}.patch.gz
		)
		eurephia? (
			mirror://sourceforge/eurephia/openvpn-2.1.0_eurephia.patch
		)"
...
src_unpack() {
	...
	use ipv6 && epatch "${WORKDIR}"/openvpn-2.1_rc22-ipv6-${IPV6_VERSION}.patch
	use eurephia && epatch "${DISTDIR}"/openvpn-2.1.0_eurephia.patch
	sed -i \
