...
src_unpack() {
	...
	if use amd64 ; then
		tar -xzf "${WORKDIR}/DesktopVideo_${PV}/other/x86_64/desktopvideo-${MY_RC}-x86_64.tar.gz"
		KSRCDIR="${WORKDIR}/${AMD64_BM_PACKAGE}/usr/src/blackmagic-${MY_RC}/"
		UDEVRULES="${WORKDIR}/${AMD64_BM_PACKAGE}"
		LIBS="${WORKDIR}/${AMD64_BM_PACKAGE}/usr/lib"
	else
		tar -xzf "${WORKDIR}/DesktopVideo_${PV}/other/i386/dektopvideo-${MY_RC}-i386.tar.gz"
		KSRCDIR="${WORKDIR}/${X86_BM_PACKAGE}/usr/src/blackmagic-${MY_RC}/"
...
src_install() {
	...
	doins ${UDEVRULES}/etc/udev/rules.d/55-blackmagic.rules
