...
LICENSE="GPL-2"
SLOT="0"
IUSE="api custom-cflags debug doc flask hvm qemu openvswitch pygrub screen xend"
...
src_prepare() {
	...
	epatch "${FILESDIR}/${PN}-4.1.2-pyxml.patch"
	
	# ipv6-support in xen-scripts
	epatch "${FILESDIR}"/${PN}-4.0.0-vif-ipv6.patch
	
	# add openvswitch hotplug scripts
	if use openvswitch; then
		epatch "${FILESDIR}/${PN}-openvswitch.patch"
	fi
	
	# set default xend network-script setting to /bin/true
	epatch "${FILESDIR}/${PN}-xend-config.patch"
}
...
#	dodoc README docs/README.xen-bugtool docs/ChangeLog
	...
	newinitd "${FILESDIR}"/xendomains.initd-r3 xendomains
